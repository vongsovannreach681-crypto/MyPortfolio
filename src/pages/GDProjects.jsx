...
const ProjectCard = ({ item, index, onClick }) => {
  ...
  return (
    <motion.div 
      ...
      className="group relative bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden hover:border-white/20 transition-colors duration-300 shadow-lg cursor-pointer break-inside-avoid"
    >
      <div className="w-full bg-black/30 relative flex items-center justify-center min-h-[250px]">
        ...
        {item.isPdf ? (
          <div className={`w-full h-[400px] relative bg-white overflow-hidden transition-opacity duration-700 z-10 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
            <iframe 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#06060a]/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-30"></div>
      </div>
...
const GDProjects = () => {
  ...
  const projectsData = useMemo(() => {
    const modules = import.meta.glob('../assets/GDFolder/**/*.{png,jpg,jpeg,pdf}', { eager: true });
  ...
  return (
    <div className="bg-[#06060a] min-h-screen flex flex-col text-white/92 relative">
      <Header />

      <main className="flex-grow pt-24 pb-20">
        {/* Banner Section */}
        <section className="relative w-full py-20 lg:py-24 overflow-hidden border-b border-white/10">
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-12 relative z-10 text-center">
            ...
            <h1 className="text-5xl md:text-7xl font-semibold tracking-[-0.04em] text-white mb-6 leading-tight">
              Graphic <span className="text-white/70">Design</span>
            </h1>
            <p className="text-lg md:text-xl text-white/60 max-w-2xl mx-auto font-light leading-[1.6]">
              A showcase of my creative graphic design works, including posters, cards, PDFs, and more.
            </p>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 sm:px-12 py-16 space-y-24">
          {projectsData.length === 0 && (
            <div className="w-full flex items-center justify-center min-h-[30vh] border border-dashed border-white/20 rounded-2xl bg-white/[0.02]">
              <p className="text-white/40 font-light text-lg">No projects found in the GDFolder.</p>
            </div>
          )}
      ...
      <AnimatePresence>
        {selectedItem && (
          ...
          >
            {/* Close Button */}
            <button 
              className="absolute top-6 right-6 sm:top-8 sm:right-10 z-50 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors backdrop-blur-md"
              onClick={(e) => {
                e.stopPropagation();
